...
with tab1:
    ...
    with c_src:
        ...
        if source_mode == "從已採收的檔案選取":
            ...
            # 檔名搜尋過濾器
            search_query = st.text_input("🔍 搜尋檔名 (關鍵字)", "").strip().lower()
            if search_query:
                files = [f for f in all_files if search_query in os.path.basename(f).lower()]
            else:
                files = all_files
    ...
    with col_m:
        model_choice = st.selectbox("選擇 AI 模型", [
            "gpt-5.4 (OpenAI 目前最強)", 
            "gpt-4o (OpenAI 原廠 直連)", 
    ...
    if getattr(st.session_state, "gen_started", False) and transcript_text:
        ...
        diag_container.caption(f"📂 讀取設定檔: `{os.path.basename(env_path)}` | 🔑 目前 Google Key: `{g_key_display}`")

        result_box = st.empty()
        full_result = ""
        
        try:
            for chunk in utils.generate_sor_stream(
            ...
            # 儲存策略文檔以供 Step 3 使用 - 統一命名規則：_strategy_{model_name}.txt
            from core.config import SOURCE_DIR
            out_file = f"{video_title}_strategy_{model_choice}.txt"
