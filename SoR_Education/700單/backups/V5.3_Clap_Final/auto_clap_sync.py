...
def calculate_expected_scenes(row):
    ...
    # 2. 判斷音標組合有幾排
    try:
        data = ast.literal_eval(str(row.get('音標組合', '[]')))
        valid_rows = [r for r in distribute_to_three_rows(data) if len(r) > 0]
        row_count = len(valid_rows)
    except:
        row_count = 1
        
    if row_count >= 1: scenes.append("T_Row1")
    if row_count >= 2: scenes.append("T_Row2")
    if row_count >= 3: scenes.append("T_Row3")
...
def process_sync():
    ...
    # 重置時間戳欄位
    for col in ["T_Matrix", "T_Row1", "T_Row2", "T_Row3", "T_Sent1", "T_Img1", "T_Sent2", "T_Img2"]:
