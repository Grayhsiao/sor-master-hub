...
def search_words():
    ...
    sort_mode = request.args.get('sort', 'sor') # 'sor' (音典) or '700'
    ...
    if grade == 'junior':
        where_clauses.append("grade_junior IS NOT NULL")
    elif grade == 'gept':
        where_clauses.append("grade_gept IS NOT NULL")
    ...
    try:
        ...
        for row in rows:
            ...
            if item.get('minum'):
                try:
                    item['minum'] = json.loads(item['minum'])
                except:
                    pass
            results.append(item)
